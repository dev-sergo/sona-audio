...
### 2. Music Generation 🧪 *(WIP — wired in-repo, backing server not deployed)*
...
> is built but not deployed: `/generate` → job queue → `acestep_service` HTTP client →
> standalone ACE-Step API server (`ACESTEP_URL`, :8002). That server isn't running in this
> snapshot, so jobs don't complete. See [README → Status](../README.md#status).
